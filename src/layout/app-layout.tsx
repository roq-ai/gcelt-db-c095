...
import { getColleges } from 'apiSdk/colleges';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'student', 'teacher', 'staff-member'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CollegeInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/colleges` : null),
    () => getColleges({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'College',
      path: isTenantUser ? `/colleges/view/${data?.[0]?.id}` : '/colleges',
      entity: 'college',
      service: AccessServiceEnum.PROJECT,
      icon: FiBookOpen,
    },
    { name: 'Exam', path: '/exams', entity: 'exam', service: AccessServiceEnum.PROJECT, icon: FiClipboard },
    { name: 'Notice', path: '/notices', entity: 'notice', service: AccessServiceEnum.PROJECT, icon: FiBell },
